...
@dp.callback_query_handler(text='Платежные терминалы')
...
@dp.callback_query_handler(text='Платежные карты ЭЛКАРТ')
...
@dp.callback_query_handler(text='Электронные кошельки')
async def terminals(call: CallbackQuery):
    text = """
УДОБНО и БЫСТРО погасить кредит, не выходя из дома через мобильные приложения и электронные кошельки наших партнеров.
...
@dp.callback_query_handler(text='Кассы')
async def terminals(call: CallbackQuery):
